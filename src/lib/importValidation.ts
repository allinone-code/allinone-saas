import { normalizeExcelDate } from "./xlsRowParse";

export interface ImportRowProblem { row: number; field: string; message: string }

/** Fulfillment girdisini kanonik forma getirir ("fba" → "FBA"); bilinmeyene dokunmaz. */
export function normalizeFulfillmentType(raw: unknown): string {
  const s = String(raw ?? "").trim();
  const up = s.toUpperCase();
  return up === "FBA" || up === "FBM" ? up : s;
}
...
/**
 * Para alanı üst sınırı: orders tablosundaki en dar finans kolonu
 * NUMERIC(10,2) (üst sınır 99.999.999,99). Bunu aşan değer DB'ye sığmaz
 * ve satır belirsiz bir "veritabanı aşaması" hatasıyla düşerdi; artık
 * doğrulama aşamasında net gerekçeyle raporlanır.
 */
...
export function normalizeMoney(raw: unknown): number | null {
  ...
  if (typeof raw === "number")
    return Number.isFinite(raw) && raw >= 0 && raw <= MONEY_MAX ? raw : null;
  ...
  const n=Number(s); return Number.isFinite(n)&&n>=0&&n<=MONEY_MAX?n:null;
}
...
export function validateImportRow(r: Record<string, any>, i: number): ImportRowProblem[] {
  const row = i + 1;
  const p: ImportRowProblem[] = [];

  const order = String(r.orderNumber ?? "").trim();
  if (!order) p.push({ row, field: "Sipariş No (Orderno)", message: "Sipariş numarası boş." });
  if (!String(r.asin ?? "").trim()) p.push({ row, field: "ASIN", message: "ASIN boş." });

  // Tarih: boş → sunucu bugünü yazar; dolu ama çözülemez → satırı DÜŞÜRMEK
  // yerine net gerekçeyle raporla (eski davranış "46043" serilerini sessizce tarihe yazıyordu)
  const dateRaw = String(r.orderDate ?? "").trim();
  if (dateRaw && !normalizeExcelDate(dateRaw)) {
    p.push({
      row,
      field: "Tarih",
      message: `"${dateRaw}" tarihe çevrilemedi. Beklenen: YYYY-AA-GG, GG.AA.YYYY, GG/AA/YYYY veya Excel tarih hücresi.`,
    });
  }

  const q = normalizeCount(r.quantity) ?? 1;
  if (r.quantity != null && String(r.quantity).trim() !== "" && (normalizeCount(r.quantity) === null || q < 1))
    p.push({ row, field: "Ürün adedi", message: "Adet pozitif bir tam sayı olmalı." });
  const shipped = normalizeCount(r.shippedToAmazon) ?? 0;
  if (shipped > q)
    p.push({ row, field: "Amazona gönderilen adet", message: `Sevk edilen adet (${shipped}) sipariş adedinden (${q}) fazla olamaz.` });
  const fire = ["p1CancelQty", "p2MissingQty", "p3DefectiveQty", "p4ExpiredQty"].reduce((n, k) => n + (normalizeCount(r[k]) ?? 0), 0);
  if (fire > q) p.push({ row, field: "P1–P4 fire adetleri", message: `Fire toplamı (${fire}) sipariş adedini (${q}) aşıyor.` });

  // fulfillmentType case-insensitive normalize edilerek denenir ("fba" geçerli)
  ([
    [normalizeFulfillmentType(r.fulfillmentType), FULFILLMENT_TYPES, "FBM/FBA"],
    [r.pshStatus, PSH_STATUSES, "PSH durumu"],
    [r.inventoryLabStatus, INVENTORY_LAB_STATUSES, "Inventory Lab durumu"],
  ] as const).forEach(([v, a, l]) => {
    const x = enumCheck(v, a as readonly string[], l as string, row);
    if (x) p.push(x);
  });
  ...
  for (const [k, l] of [
    ...
    ["correctedCost", "Düzeltilmiş maliyet"],
    ["refundAmount", "Refund miktarı"],
  ] as const)
    if (r[k] != null && String(r[k]).trim() !== "" && normalizeMoney(r[k]) === null)
      p.push({ row, field: l, message: `"${r[k]}" sayıya çevrilemedi.` });
  return p;
}
